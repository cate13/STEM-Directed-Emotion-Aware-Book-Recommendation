...
def generate_user_prompts(data):
    ...
    Uses all books in candidate_profile as context, with a priority on STEM.
    """
    all_prompts = []
    for user in data:
...
- PRIMARY PRIORITY: Prioritize books about STEM topics (Science, Technology, Engineering, and Mathematics). Even if the user has read other genres, look for overlapping themes in the candidates that lean toward technical, analytical, or scientific subjects.
- Consider the themes, tone, and genres of the user's highly-rated books as secondary context.
- Factor in the user's emotional preferences when choosing between two similar STEM or non-STEM titles.

IMPORTANT CONSTRAINTS:
- STEM PRIORITIZATION: Titles involving science, math, or technology should generally appear higher in the ranking than  non-STEM subjects, unless the STEM title is completely unrelated to the user's profile.
...
"""
    ...
    print("all prompts made with STEM priority")
    return all_prompts
...
def main():
    ...
    output_file = f'with_stem_instructions/{model_file_name}_user_rankings_results.json'
